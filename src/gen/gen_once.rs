...
pub trait GenOnce<T> {
    ...
    fn map_once<U, F>(self, f: F) -> adapters::MapOnce<T, U, Self, F>
    where
        Self: Sized,
        F: FnOnce(T) -> U,
    {
        adapters::MapOnce::new(self, f)
    }

    /// Creates a new `GenOnce` whose value is generated by the generated `GenOnce` of `self`.
    fn flatten_once<U>(self) -> adapters::FlattenOnce<U, T, Self>
    where
        Self: Sized,
        T: GenOnce<U>
    {
        adapters::FlattenOnce::new(self)
    }
    ...
    fn flat_map_once<U, GU, F>(self, f: F) -> adapters::FlatMapOnce<T, U, Self, GU, F>
    where
        Self: Sized,
        GU: GenOnce<U>,
        F: FnOnce(T) -> GU,
    {
        adapters::FlatMapOnce::new(self, f)
    }
